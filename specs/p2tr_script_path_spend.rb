...
internal_pubkey = '0908421cb37d204b0c68660d093534d50d01fa791a3313e5fd9c21da137785eb'
puts "internal_pubkey", internal_pubkey

# -----------
# Custom Tweak
# -----------

rootstock_deposit_address = ["4C9a9CbFa14106439B0F96a64d9260F3b8947934"].pack("H*")
value = 100_000  # 0.001 BTC
packed_data = rootstock_deposit_address + [value].pack('Q>')  # 'Q>' for big-endian 64-bit unsigned integer

# Calculate SHA-256 hash
packed_data_hash = Digest::SHA256.digest(packed_data)

# Debug logging (equivalent to console.log in Solidity)
puts "customTweak:"
puts packed_data_hash.unpack1('H*')  # Convert to hex string for display
...
puts "message", internal_pubkey + merkle_root

# create a hash from the public key and merkle root - this will be used to tweak the public key
# tweak = tagged_hash("TapTweak", internal_pubkey + merkle_root + packed_data_hash)
