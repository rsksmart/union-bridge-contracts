...
contract MemberRegistryTest is Test, HelperContract {
    ...
    function test_applyToStream_Success_PausedContract() external {
        ...
        vm.deal(registryAddress, 1 ether);

        // Assert
        vm.expectEmit(address(memberRegistry));
        emit IMemberRegistry.NewSecurityBondDeposit(member, denomination, role, minimumDeposit);

        // Act
        vm.prank(registryAddress);
        memberRegistry.applyToStream{value: minimumDeposit}(
            member, denomination, role, memberRegistrationKeys, generateDefaultUTXO()
        );
    }

    function test_unsubscribeFromStream_Success_PausedContract() external {
        // Arrange
    ...
    function test_withdrawAvailableBalance_Success_UnpausedContract() external {
        ...
        // Assert
        vm.expectEmit(address(memberRegistry));
        emit IMemberRegistry.AvailableBalanceRetrieved(member, amount);
    ...
    function test_setReApplyForStream_Success_UnpausedContract() external {
        // Arrange
        uint256 privKey = uint256(1);
        address member = vm.addr(privKey);
        memberRegistrationKeys = generateRegistrationPublicKeys(privKey);
        ...
        // Assert
        vm.expectEmit(address(memberRegistry));
        emit IMemberRegistry.MemberReApplyUpdated(member, denomination, reApply);
    ...
    function test_applyToStream_Revert_PublicKeyMismatch_DISPUTE() external {
        // Arrange
        uint256 privKey = uint256(1);

        // Create fresh keys for second registration with different DISPUTE key only
        MemberRegistrationKeys memory differentPubKey;
        differentPubKey.takeKey = memberRegistrationKeys.takeKey; // Same TAKE key
        differentPubKey.disputeKey = generateECDSAPublicKey(privKey + 1, PublicKeyType.DISPUTE); // Different DISPUTE key
    ...
    function test_applyToStream_Revert_PublicKeyMismatch_COMMUNICATION() external {
        ...
        MemberRegistrationKeys memory differentPubKey;
        differentPubKey.takeKey = memberRegistrationKeys.takeKey; // Same TAKE key
        differentPubKey.disputeKey = memberRegistrationKeys.disputeKey; // Same DISPUTE key
    ...
    function test_applyToStream_Revert_InvalidEDCSAPublicKey_X_NotOnCurve_DISPUTE() external {
        // Arrange
        uint256 privKey = uint256(1);
        address user = vm.addr(privKey);
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, DEFAULT_ROLE);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                memberRegistrationKeys.disputeKey.publicKeyY
            )
        );
        // Act
        vm.prank(user);
    ...
    function test_applyToStream_Revert_InvalidEDCSAPublicKey_Y_NotOnCurve_DISPUTE() external {
        // Arrange
        uint256 privKey = uint256(1);
        address user = vm.addr(privKey);
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, DEFAULT_ROLE);
    ...
    function test_applyToStream_Revert_InvalidZeroEDCSASignature_V_DISPUTE() external {
        // Arrange
        uint256 privKey = uint256(1);
        address user = vm.addr(privKey);
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, DEFAULT_ROLE);
        ...
        // Set the signature V to 0
        memberRegistrationKeys.disputeKey.v = 0;
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                PublicKeyType.DISPUTE,
                memberRegistrationKeys.disputeKey
            )
        );
        // Act
        vm.prank(user);
    ...
    function test_applyToStream_Revert_InvalidZeroEDCSASignature_R_DISPUTE() external {
        // Arrange
        uint256 privKey = uint256(1);
        address user = vm.addr(privKey);
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, DEFAULT_ROLE);
        ...
        // Set the signature R to 0
        memberRegistrationKeys.disputeKey.r = bytes32(0);
    ...
    function test_applyToStream_Revert_InvalidZeroEDCSASignature_S_DISPUTE() external {
        // Arrange
        uint256 privKey = uint256(1);
        address user = vm.addr(privKey);
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, DEFAULT_ROLE);
        ...
        // Set the signature S to 0
        memberRegistrationKeys.disputeKey.s = bytes32(0);
    ...
    function test_applyToStream_Revert_ECDSAInvalidSignature_V_TAKE() external {
        ...
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, DEFAULT_ROLE);
        vm.deal(user, minimumDeposit);
        MemberRegistrationKeys memory incorrectPubKeysRegistration = memberRegistrationKeys;
    ...
    function test_applyToStream_GasConsumptionCheck() external {
        ...
        uint256 privKey = uint256(1);
        address user = vm.addr(privKey);
        MemberRegistrationKeys memory memberRegistrationKeysMemory = memberRegistrationKeys;
        uint256 minimumDeposit = streamManager.getMinimumDeposit(DEFAULT_STREAM, role);
        vm.deal(user, minimumDeposit);
    ...
    function setup_applyToStream(
        address _user,
        MemberRegistrationKeys memory _memberRegistrationKeys,
        StreamDenomination _stream,
        Role _requestedRole
    ) internal returns (uint256) {
        // Arrange
        address[] memory committeesCandidates = memberRegistry.getCommitteeCandidates(_stream, _requestedRole);
        ...
        committeesCandidates = memberRegistry.getCommitteeCandidates(_stream, _requestedRole);
        assertEq(
        ...
        assertEq(
            ...
            _user,
            "candidate address should match member address"
        );
    ...
    function test_applyToStream_Success_OddParityDisputeKey() external {
        MemberKeys memory keys = setup_memberWithOddParityCovenantKey();
    ...
    function test_getMemberDisputePubKey_Revert_MemberNotRegistered() external {
        // Arrange
        address user = vm.addr(uint256(100));
        ...
        // Act
        memberRegistry.getMemberDisputePubKey(user);
    }

    function test_getMemberDisputePubKey_Success() external {
        // Arrange
        uint256 privKey = uint256(1);
        address userAddress = vm.addr(privKey);
        MemberKeys memory pubKeys = memberKeysFromRegistration(memberRegistrationKeys);
