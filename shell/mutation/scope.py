...
def ignore() -> list[str]:
    """[targets].ignore substrings from mewt.toml. Missing the key is a config error."""
    targets = _load_config().get("targets", {})
    if "ignore" not in targets:
        sys.exit(f"{CONFIG} is missing [targets].ignore")
    return list(targets["ignore"])
...
def active_contracts() -> list[str]:
    ...
    excluded = ignore()
    return [path for path in paths if not any(substr in path for substr in excluded)]
